...
b8 physical_device_meets_requirements(
  ...
  if ((!requirements->graphics || (requirements->graphics && out_queue_familiy_info->graphics_family_index != -1)) &&
      ...
      if(available_extension_count != 0) {
        ...
        for(u32 i = 0; i<required_extension_count; i++) {
          ...
          if(!found) {
            ...
            kfree(
              available_extensions, sizeof(VkExtensionProperties) * available_extension_count,
              MEMORY_TAG_RENDERER);
            return FALSE;
          }
        }
